...
/**
 * \file    mydata/admin/setup.php
 * \ingroup mydata
 * \brief   MyDATA setup page.
 */
...
require_once '../lib/mydata.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "mydata@mydata"));
...
$arrayofparameters = array(
		...
		//'MYDATA_CLASSIFICATION_CATEGORY'=>array('type'=>'string', 'css'=>'minwidth300' ,'enabled'=>1),
		...
		//'MYDATA_MYPARAM4'=>array('type'=>'emailtemplate:thirdparty', 'enabled'=>1),
		//'MYDATA_MYPARAM33'=>array('type'=>'integer', 'label'=>'Status', 'enabled'=>1, 'visible'=>1, 'notnull'=>1, 'position'=>20, 'showoncombobox'=>true, 'arraykeyval'=>array('0'=>'Draft','1'=>'Active','-1'=>'Cancel')),
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/mydata/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'MYDATA_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'MYDATA_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$head = mydataAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "mydata@mydata");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("MyDATASetupPage").'</span><br><br>';
...
//		echo get_include_path();
		...
		echo "MySQLi: ", extension_loaded('mysqli') ? 'Found' : 'Missing', '<br>';

		require_once "HTTP/Request2.php";
		if (class_exists('HTTP_Request2')) {
			print("HTTP_Request2: Found");
		} else  print("HTTP_Request2: Missing");

		print "<br>";
                echo "<br>";
		print '<a href="https://www.aade.gr/sites/default/files/2021-09/myDATA%20API%20Documentation_ERP_v1.0.3_official.pdf">Πατήστε εδώ για το PDF myDATA Ηλεκτρονικά Βιβλία ΑΑΔΕ</a>';
		print '<br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'MYDATA_'.strtoupper($myTmpObjectKey).'_ADDON';
